defmodule CoreWeb.Utils.Paginate do
  @moduledoc false
  ...
  def get_limit(limit_param) do
    limit = to_int(limit_param || Application.get_env(:core, :query)[:default_limit])
    limit = if limit > @min_limit, do: limit, else: @min_limit
    max_limit = Application.get_env(:core, :query)[:max_limit]
    if limit > max_limit, do: max_limit, else: limit
  end

  defp get_limit_value(params) do
    {limit, params} =
      Keyword.pop(params, :limit, Application.get_env(:core, :query)[:default_limit])
