...
defmodule Acl.AclContext do
  ...
  alias Acl.AclContext.Res
  alias Acl.AclContext.Role
  alias Acl.AclContext.Rule
  ...
  def get_rule_by(%{"role" => role, "res" => res, "action" => nil, "permission" => nil}) do
    case Repo.one(
           from r in Rule,
             where:
               r.role == ^role and r.res_id == ^res and is_nil(r.action) and is_nil(r.permission),
             preload: [
               :res
             ]
         ) do
  ...
  def get_rule_by(%{"role" => role, "res" => res, "action" => action, "permission" => nil}) do
    case Repo.one(
           from r in Rule,
             where:
               r.role == ^role and r.res_id == ^res and r.action == ^action and
                 is_nil(r.permission),
             preload: [:res]
         ) do
      nil -> {:error, :rule_not_found}
  ...
  def get_rule_by(%{"role" => role, "res" => res, "permission" => permission, "action" => action}) do
    if is_nil(action) do
      case Repo.one(
             from r in Rule,
               where:
                 r.role == ^role and r.res_id == ^res and is_nil(r.action) and
      ...
      case Rule
           |> Repo.get_by(role: role, res_id: res, permission: permission, action: action) do
        nil -> {:error, :rule_not_found}
        rule -> rule
      end
    end
