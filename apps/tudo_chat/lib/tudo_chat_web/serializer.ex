if Code.ensure_loaded?(SweetXml) do
  defmodule TudoChatWeb.Serializer do
    @moduledoc false
    ...
    def attribute_list_to_map(list_of_maps, convert_to_atoms) do
      ...
      |> Enum.reduce(%{}, fn %{name: name, value: val}, acc ->
        attribute_name = name |> Macro.underscore()
    ...
    def parse({:ok, _} = rsp, :change_message_visibility) do
    ...
    def parse({:ok, _} = rsp, :delete_message) do
      parse_request_id(rsp, ~x"//DeleteMessageResponse")
    end

    def parse({:ok, _} = rsp, :delete_queue) do
    ...
    FIXME - Cast :sender_fault to boolean
    """
